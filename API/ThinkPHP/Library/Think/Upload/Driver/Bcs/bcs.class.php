...
class BaiduBCS {
	...
	/**
	 * 通过此方法以拷贝的方式创建object，object来源为$course
	...
	public function copy_object($source, $dest, $opt = array()) {
		...
		$opt [self::METHOD] = 'PUT';
		self::set_header_into_opt ( 'x-bs-copy-course', 'bs://' . $source [self::BUCKET] . $source [self::OBJECT], $opt );
		if (isset ( $opt ['source_tag'] )) {
			self::set_header_into_opt ( 'x-bs-copy-course-tag', $opt ['source_tag'], $opt );
		}
		$response = $this->authenticate ( $opt );
