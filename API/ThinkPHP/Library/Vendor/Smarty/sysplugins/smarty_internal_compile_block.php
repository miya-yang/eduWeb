...
class Smarty_Internal_Compile_Block extends Smarty_Internal_CompileBase {
    ...
    /**
     * Save or replace child block course by block name during parsing
     *
     * @param string $block_content     block course content
     * @param string $block_tag         opening block tag
     * @param object $template          template object
     * @param string $filepath          filepath of template course
     */
            ...
            if ($_match[8] != 'hide' || isset($template->block_data[$_name])) {        // replace {$smarty.block.child}
                if (strpos($block_content, $template->smarty->left_delimiter . '$smarty.block.child' . $template->smarty->right_delimiter) !== false) {
                    if (isset($template->block_data[$_name])) {
                        ...
                        $template->block_data[$_name]['course'], $block_content);
                ...
                if (isset($template->block_data[$_name])) {
                    if (strpos($template->block_data[$_name]['course'], '%%%%SMARTY_PARENT%%%%') !== false) {
                        $template->block_data[$_name]['course'] =
                        str_replace('%%%%SMARTY_PARENT%%%%', $block_content, $template->block_data[$_name]['course']);
                    } elseif ($template->block_data[$_name]['mode'] == 'prepend') {
                        $template->block_data[$_name]['course'] .= $block_content;
                    } elseif ($template->block_data[$_name]['mode'] == 'append') {
                        $template->block_data[$_name]['course'] = $block_content . $template->block_data[$_name]['course'];
                    }
                } else {
                    $template->block_data[$_name]['course'] = $block_content;
    ...
    /**
     * Compile saved child block course
     *
     * @param object $compiler  compiler object
    ...
    public static function compileChildBlock($compiler, $_name = null)
    {
        ...
        // undefined child?
        if (!isset($compiler->template->block_data[$_name]['course'])) {
            return '';
        }
        $_tpl = new Smarty_Internal_template ('string:' . $compiler->template->block_data[$_name]['course'], $compiler->smarty, $compiler->template, $compiler->template->cache_id,
        ...
        if (strpos($compiler->template->block_data[$_name]['course'], '%%%%SMARTY_PARENT%%%%') !== false) {
...
class Smarty_Internal_Compile_Blockclose extends Smarty_Internal_CompileBase {
    ...
    public function compile($args, $compiler)
    {
        ...
        if (isset($compiler->template->block_data[$_name]) && !isset($compiler->template->block_data[$_name]['compiled'])) {
            ...
            if (isset($saved_data[0]['hide']) && !isset($compiler->template->block_data[$_name]['course'])) {
                $_output = '';
            } else {
